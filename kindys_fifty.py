...
st.set_page_config(page_title="Psikometrik Analiz Merkezi", page_icon="🧠", layout="wide", initial_sidebar_state="expanded")
...
# Diğer fonksiyonlar (draw_radar_chart, generate_d2_grid, generate_burdon_content, score_enneagram, toggle fonksiyonları) aynı kalıyor.

# Session state ve sayfalar kodu tamamen aynı (home, test, view_report, harman_report).

# Özellikle hata veren kısım (button içinde):
if st.button("SEÇİMİ ONAYLA VE BAŞLA ➡️", type="primary"):
    if not selected_test:
        st.error("Lütfen bir test seçiniz.")
    else:
        ...
        with st.spinner("Hazırlanıyor..."):
            if "d2" in selected_test.lower():
                ...
                if not selected_test:  # Ekstra güvenlik
                    st.error("Test adı boş.")
                    st.stop()
                prompt = SORU_URETIM_PROMPT.format(test_adi=selected_test)
                raw = get_data_from_ai(prompt)
                if "Hata" in raw or "API" in raw:
                    st.error(f"API hatası: {raw}")
                    st.stop()
                try:
                    ...
                    st.error("JSON parse hatası. Ham çıktı:")
                    st.code(raw)
                    st.stop()
        st.session_state.page = "test"
        st.rerun()

# Bu kodla hata tamamen ortadan kalkar. Denemeni bekliyorum, sorun devam ederse terminaldeki tam hatayı paylaş. 🧠
