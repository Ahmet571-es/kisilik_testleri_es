                ...
                with st.spinner("Test hazırlanıyor, lütfen bekleyiniz..."):
                    if "d2" in selected_test.lower():
                        ...
                        try:
                            ...
                            st.error("Soru üretimi başarısız. API yanıtı JSON formatında değil.")
                            st.stop()
...
# --- RAPOR ---
    ...
    with col1:
        if st.button("🏠 Ana Sayfa", use_container_width=True):
            ...
            st.rerun()
    with col2:
        if st.button("🔄 Yeni Test Yap", use_container_width=True):
            st.session_state.page = "home"
            st.session_state.intro_passed = False
            st.rerun()
    with col3:
        if len(st.session_state.results) > 1:
            if st.button("🧩 Bütüncül Rapor Al", use_container_width=True):
                st.session_state.page = "harman_report"
                st.rerun()
...
# --- HARMAN RAPOR ---
    ...
    if st.button("HARMAN RAPOR OLUŞTUR", type="primary", use_container_width=True):
