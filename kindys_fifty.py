...
def generate_gardner_questions():
    return [{"id": i, "text": f"Soru {i}: Bu alandaki becerilerim veya ilgilerim gelişmiştir.", "area": "Genel"} for i in range(1, 80)]
...
def generate_sperry_questions():
    return [{"id": i, "text": f"Soru {i}: Karar verirken duygularımdan ziyade mantığıma güvenirim."} for i in range(1, 21)]
...
def generate_dusko_questions():
    return [{"id": i, "text": f"Soru {i}: Sınav anında bildiklerimi unuturum."} for i in range(1, 23)]
...
for link in citations: st.markdown(f"- {link}") bu kod bloğunu verdiğin normatif veriler ile güncelle. normatif verileri de rapor kısmına ekle. rapor kısmında normatif veriler ile karşılaştırma olsun. rapor kısmında stabilite katsayısı olsun. raporlar bireysel ve harmanlı olsun.
